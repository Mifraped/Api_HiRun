...
const getNovedades = (req, res) => {
	pool
		.execute(
			'SELECT business.*, users.name AS providerName, users.surname AS providerSurname, users.photo AS userPhoto, GROUP_CONCAT(service.description) AS services, GROUP_CONCAT(service.price) AS prices FROM business JOIN users ON business.provider = users.id_user JOIN service ON business.id_business = service.id_business GROUP BY business.id_business ORDER BY business.create_date DESC LIMIT 10'
		)
		.then(([results, fields]) => {
			res.json(results);
		})
...
FROM
  ...
  LEFT JOIN business_cat ON business.id_business = business_cat.business
...
WHERE 1=1`;
	...
	if (rating) {
		...
		query += ` AND business.rating >= ? AND business.rating < ?`;
		queryParams.push(
			ratingNumber.toString(),
			(Math.floor(ratingNumber) + 1 - 0.01).toString()
		);
	}
