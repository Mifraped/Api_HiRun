...
const getBusiness = async (req, res) => {
	try {
		...
		if (req.query.id_user) {
			...
			sql =
				'SELECT title, bus.photo, rating, bus.id_business, bus.provider FROM hirun.users AS us INNER JOIN hirun.business AS bus ON (us.id_user = bus.provider) WHERE us.id_user = ? ORDER BY bus.id_business DESC';
			...
			sql =
				'SELECT business.*, users.name AS providerName, users.surname AS providerSurname, users.photo AS userPhoto, service.price, service.description FROM business JOIN users ON business.provider = users.id_user JOIN service ON business.id_business = service.id_business WHERE business.rating >= ?';
		} else {
			//todos
			params = [];
			sql =
				'SELECT business.*, users.name AS providerName, users.surname AS providerSurname, users.photo AS userPhoto, GROUP_CONCAT(service.description) AS description, MIN(service.price) AS price FROM business JOIN users ON business.provider = users.id_user JOIN service ON business.id_business = service.id_business GROUP BY business.id_business ORDER BY business.create_date DESC LIMIT 50';
		}
		let [result] = await pool.query(sql, params);
		let respuesta = {
			error: false,
...
const putBusiness = async (req, res) => {
	try {
		let params = [
			...
			req.body.photo,
			req.body.address,
			req.body.id_business,
		];
		let sql =
			'UPDATE business SET title = COALESCE(?, title), photo = COALESCE(?, photo), address = COALESCE(?, address) WHERE id_business = ?';
		let [result] = await pool.query(sql, params);
		let answer = {
			error: false,
			code: 200,
