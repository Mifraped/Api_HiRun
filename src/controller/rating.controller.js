...
const getRating = async (req, res) => {
	try {
		...
		if (req.query.id_user && req.query.id_service) {
			...
			sql = "SELECT id_rate, id_user , AVG(rate) AS rate, comment FROM rate AS r JOIN service AS s ON r.id_service = s.id_service JOIN business AS b ON  s.id_business = b.id_business WHERE b.id_business = ?";
		}
